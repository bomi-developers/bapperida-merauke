<x-layout>
    ...
    <main class="p-6">
        ...
        {{-- Tabel Kategori --}}
        <section class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-md overflow-hidden">
                            ...
                            <tr id="kat-row-{{ $kat->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center space-x-4">
                                        <button class="edit-btn font-medium text-indigo-600 dark:text-indigo-500 hover:underline" title="Edit" data-id="{{ $kat->id }}">
                                            <i class="bi bi-pencil-square text-base"></i>
                                        </button>
                                        <button class="delete-btn font-medium text-red-600 dark:text-red-500 hover:underline" title="Hapus" data-id="{{ $kat->id }}">
            ...
            <form id="kategori-form">
                ...
                <div class="p-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_kategori" name="nama_kategori" class="form-input w-full" required>
                    </div>
                </div>
                <div class="p-6 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (form) {
                form.addEventListener('submit', async (e) => {
                    ...
                    const data = { nama_kategori: formData.get('nama_kategori') };

                    const response = await fetch(form.action, {
                        method: isUpdating ? 'POST' : 'POST', // Tetap POST, method asli di-handle oleh _method
                        body: JSON.stringify(isUpdating ? { ...data, _method: 'PUT' } : data),
                        headers: {
                    ...
                    if (!response.ok) {
                        ...
                        Swal.fire({ icon: 'error', title: 'Gagal Validasi', html: `<ul class="text-left list-disc list-inside">${errorMessages}</ul>` });
                        return;
                    }
    ...
    <style>
        .show-btn i, .edit-btn i, .delete-btn i { pointer-events: none; }
        .form-label { display: block; margin-bottom: 0.5rem; font-weight: 500; color: #374151; }
        .dark .form-label { color: #d1d5db; }
        .form-input { border: 1px solid #d1d5db; border-radius: 0.5rem; padding: 0.5rem 0.75rem; width: 100%; background-color: #f9fafb; color: #111827; }
        .dark .form-input { border-color: #4b5563; background-color: #374151; color: #f9fafb; }
        .btn-primary { background-color: #4f46e5; color: white; padding: 0.6rem 1.2rem; border-radius: 0.5rem; font-weight: 500; }
        .btn-secondary { background-color: #e5e7eb; color: #1f2937; padding: 0.6rem 1.2rem; border-radius: 0.5rem; font-weight: 500; }
