@forelse ($beritas as $berita)
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <td class="px-6 py-4">
            <span class="text-indigo-700 dark:text-indigo-400">{{ $berita->author->name ?? 'N/A' }}</span>
            <br><small>Pada : {{ $berita->created_at->format('d F Y') }}</small>
        </td>
        <td class="px-6 py-4">
            <span
                class="px-2 py-1 text-xs font-medium rounded-full {{ $berita->status == 'published' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' }}">
        ...
        <td class="px-6 py-4">
            <span
                ...
                {{ $berita->views_count > 0 ? number_format($berita->views_count) . ' x' : 'Belum ada' }}
            </span>
        </td>
