...
<script>
    let pageUrl = "{{ route('admin.opd.data') }}";
                        ...
                        html += `
                            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                                ...
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-200">
                                    ${b.user 
                                        ? `<b>${b.user.name}<b><br><small class="bg-green-200 dark:bg-green-600 px-2 py-1 text-green-800 dark:text-green-200 rounded-xl">${b.user.email}</small>` 
    ...
    function userPegawai(pegawai) {
        ...
        document.getElementById('username').value = pegawai.name;
        document.getElementById('email').value = pegawai.email;
        document.getElementById('role').value = "opd";
        document.getElementById('statusAkun').innerHTML = "Memeriksa akun...";

        fetch(`/admin/akun/opd/${id}`)
    ...
    function editPegawai(data) {
        document.getElementById('modalTitle').innerText = "Edit Pegawai Perangkat Daerah";
        document.getElementById('pegawai_id').value = data.id ?? '';
        document.getElementById('name').value = data.name ?? '';
        document.getElementById('email_opd').value = data.email ?? '';
    ...
    document.getElementById('formPegawai').addEventListener('submit', function(e) {
        ...
        const url = id ? `/admin/opd/${id}` : `/admin/opd`;

        const data = {
            name: document.getElementById('name').value,
            nip: document.getElementById('nip').value,
            no_hp: document.getElementById('no_hp').value,
            email: document.getElementById('email_opd').value,
