@php
    ...
    // 1. Default Fallback (Gambar bawaan template jika belum ada settingan apa-apa)
    $bgImage = asset('assets/image6.jpg');

    // 2. Cek Database: Apakah ada gambar khusus untuk route ini?
    $pageHero = \App\Models\PageHero::where('route_name', $currentRoute)->first();

    // 3. Cek Database: Apakah ada gambar Default Global?
    $globalSetting = \App\Models\WebsiteSetting::first();

    if ($pageHero && $pageHero->hero_bg) {
        // Prioritas 1: Pakai gambar khusus halaman ini
        $bgImage = asset('storage/' . $pageHero->hero_bg);
    } elseif ($globalSetting && $globalSetting->hero_bg) {
        // Prioritas 2: Pakai gambar default dari pengaturan website
...
<section class="relative top-0 overflow-hidden h-[1200px] [clip-path:ellipse(120%_60%_at_50%_0%)]">
    ...
    {{-- LOGIKA TEKS HERO (SAMA SEPERTI SEBELUMNYA) --}}
    @php
        $heroGroups = [
            [
                'routes' => ['welcome'],
    ...
    <div class="relative z-10 h-full flex flex-col items-center justify-start text-center px-4 animate-fade-in-up"
        ...
        {{-- <form class="mt-8 w-full max-w-lg">
            <div class="flex items-center bg-white rounded-full p-2 shadow-md">
                <svg class="h-6 w-6 text-gray-400 ml-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <input type="text" placeholder="Cari inovasi, produk, atau berita..." class="w-full bg-transparent px-4 text-gray-700 focus:outline-none" />
                <button type="submit" class="bg-[#CCFF00] text-[#006FFF] px-6 py-2 rounded-full text-sm font-semibold hover:bg-yellow-400 transition-colors">
                    Cari
                </button>
            </div>
        </form> --}}
